...
export const uploadImages = (image) => (dispatch, getState) => {
    ...
    axiosApiInstance.post(`${UPLOAD}/${_id}/businesses/`, form, {
...
export const updateImagesIntoRedux = (type, payload) => (dispatch) => {
    let dispatchType = type === 'thumbnail' ? REMOVE_THUMBNAIL_IMAGES : REMOVE_GALLERY_IMAGES;
...
export const uploadGalleryImages = (payload) => (dispatch, getState) => {
    ...
    Promise.all(chunks).then((res) => {
        dispatch({type: UPLOAD_GALLERY_IMAGES_API, gallery: res, galleryLoading: false})
    }).catch(error => {
